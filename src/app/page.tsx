...
export default function Home() {
  ...
  const [ipcDataByPackage, setIPCDataByPackage] = useState<Record<string, IPCData> | null>(null);
  ...
  const loadData = async () => {
    try {
      ...
      if (complianceResponse?.ipcDataByPackage) {
        console.log('[Page] IPC data received from API:', complianceResponse.ipcDataByPackage);
        setIPCDataByPackage(complianceResponse.ipcDataByPackage);
        console.log('✅ IPC data loaded:', complianceResponse.ipcDataByPackage);
      } else {
        console.warn('[Page] ⚠️ IPC data not available in response');
        setIPCDataByPackage(null);
      }
    ...
    return (
      <main className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Fetching from 5 Google Sheets</p>
  ...
  if (tasks) {
    return (
      <div>
          ...
          <div className="container mx-auto px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-gray-700">
                <strong>{tasks.length} tasks</strong> loaded from <strong>5 Google Sheets</strong>
                {lastRefresh && (
        ...
        <Dashboard tasks={tasks} packageCompliance={packageCompliance} ipcDataByPackage={ipcDataByPackage} onReset={() => { }} />
      </div>
    );
  }
